...
def add_gems
  ...
  add_gem 'shoulda-matchers', '~> 6.1', group: %i[development test]
  add_gem 'simplecov', '~> 0.22.0', require: false, group: %i[development test]
  add_gem 'database_cleaner', '~> 2.0', '>= 2.0.2', group: %i[development test]
  add_gem 'dotenv-rails', '~> 3.0', '>= 3.0.2', groups: %i[development test]
...
def add_users
  return unless yes?('Would you like to install Devise for user management ?')
  ...
  generate :devise, @model_name, 'first_name', 'last_name', 'first_name_kana', 'last_name_kana', 'phone',
           'postalcode', 'prefecture', 'city', 'street_address'
...
def add_rspec
  ...
  gsub_file('spec/rails_helper.rb', 'ActiveRecord::Migration.maintain_test_schema!',
...
after_bundle do
  ...
  inject_into_file 'app/models/contact.rb', after: /class Contact < ApplicationRecord\n/ do
    <<-RUBY
  ...
  initializer 'rack_timeout.rb', <<-CODE
    ...
    # Optional: Disable the Rack::Timeout logger
